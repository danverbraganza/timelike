...
/**
 * Creates a 3D cube/block shape for elevated tiles
 * @param centerX Center X coordinate
 * @param centerY Center Y coordinate
 * @param hexSize Size of the hexagon base
 * @param height Height of the block
 * @returns Object with paths for top, left, and right faces
 */
export function createIsometric3DBlock(
  centerX: number,
  centerY: number,
  hexSize: number,
  height: number
  ...
  // Base hexagon at elevation 0
  const baseVertices: IsometricPoint[] = [];
  for (let i = 0; i < 6; i++) {
    const angle = (i * 60) * Math.PI / 180;
    const x = centerX + hexSize * Math.cos(angle);
    const y = centerY + hexSize * Math.sin(angle);
    baseVertices.push({ x, y, z: 0 });
  }
  
  // Top hexagon at elevation height
  const topVertices: IsometricPoint[] = baseVertices.map(v => ({ ...v, z: height }));
  
  // Convert to screen coordinates
  const baseScreen = baseVertices.map(v => worldToScreen(v));
  const topScreen = topVertices.map(v => worldToScreen(v));
  
  // Create top face (hexagon)
  const topPath = topScreen.map((point, index) => {
    const command = index === 0 ? 'M' : 'L';
    return `${command} ${point.x.toFixed(2)} ${point.y.toFixed(2)}`;
  }).join(' ') + ' Z';
  
  // Create left face (connecting vertices 0-1-2 base to top)
  const leftPath = [
    `M ${baseScreen[0].x} ${baseScreen[0].y}`,
    `L ${topScreen[0].x} ${topScreen[0].y}`,
    `L ${topScreen[1].x} ${topScreen[1].y}`,
    `L ${baseScreen[1].x} ${baseScreen[1].y}`,
    'Z'
  ].join(' ');
  
  // Create right face (connecting vertices 1-2-3 base to top)
  const rightPath = [
    `M ${baseScreen[1].x} ${baseScreen[1].y}`,
    `L ${topScreen[1].x} ${topScreen[1].y}`,
    `L ${topScreen[2].x} ${topScreen[2].y}`,
    `L ${baseScreen[2].x} ${baseScreen[2].y}`,
    'Z'
  ].join(' ');
